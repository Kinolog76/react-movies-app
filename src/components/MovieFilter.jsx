    ...
    <>
      <div className="flex justify-between items-center mx-5 mt-2">
        ...
        <div className="flex justify-center">
          <button onClick={handlePrevPage} disabled={page === 1} className="bg-blue-500 hover:bg-blue-700 disabled:bg-gray-500 text-white font-bold py-2 px-4 rounded mr-5">
            Prev
          </button>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 p-5 flex-wrap">
        {filteredData.map((film) => (
          <MovieCard key={film.id} dataFilm={film} />
        ))}
      </div>
    </>
  );
